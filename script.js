...
fetch("projects.json")
  ...
  .then((data) => {
    data.data.forEach((element) => {
      ...
      const captionArr = document.querySelectorAll(".caption");
      captionArr.forEach((caption) => {
        caption.addEventListener("click", function (event) {
          // Add class-name for full display
          caption.classList.toggle("full-display");
          console.log(caption.classList);
          let tmp = document.querySelector(".close-box");
          tmp.addEventListener("click", () => {
            caption.classList.remove("full-display");
            console.log(caption.classList);
          });
        });
      });
  ...
  projectSlides.innerHTML += `
    <figure class="project-figure" id="slide-${element.id}">
              ...
              <figcaption class="caption">
                <div class="hidden close-box">
                <?xml version="1.0" encoding="UTF-8"?>
                <svg id="close-button" width="1200pt" height="1200pt" version="1.1" viewBox="0 0 1200 1200" xmlns="http://www.w3.org/2000/svg">
